...
class WeChat
{
    ...
    public function responseMSG()
    {
        ...
        if (!empty($postArr['MsgType']) && $postArr['MsgType'] == 'event') {   //用户进入客服消息
    ...
    public function summaryTrend($ghid, $appid, $appsecret)
    {
        $this->_appid = $appid;
        ...
        $json = json_encode($data, JSON_UNESCAPED_UNICODE);
        //以'json'格式发送post的https请求
        $result = $this->_requestPost($url, $json);
        return $result;
    }
    ...
    private function _requestPost($url, $json)
    {
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_POST, 1); // 发送一个常规的Post请求
        ...
        $output = curl_exec($curl);
        if (curl_errno($curl)) {
            Log::info( 'Errno' . curl_error($curl));//捕抓异常
        }
